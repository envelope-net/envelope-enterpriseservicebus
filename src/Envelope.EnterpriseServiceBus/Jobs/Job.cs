...
namespace Envelope.EnterpriseServiceBus.Jobs;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	...
	internal void Initialize(IJobProviderConfiguration config, IServiceProvider serviceProvider)
	{
		...
		lock (_initLock)
		{
			...
			var executeResult = JobExecuteResult.CreateJobExecuteResultInternal(JobInstanceId, true, JobExecuteStatus.NONE);
	...
	private async Task<bool> OnSequentialAsyncTimerExceptionAsync(object? state, Exception exception)
	{
		...
		var executeResult = JobExecuteResult.CreateJobExecuteResultInternal(sequentialTimerState.ExecutionId, true, JobExecuteStatus.Failed);
...
public abstract class Job<TData> : Job, IJob<TData>, IJob
{
	...
	private async Task BeforeStartInternalAsync(ITraceInfo traceInfo)
	{
		...
		var jobExecuteResult = JobExecuteResult.CreateJobExecuteResultInternal(JobInstanceId, true, JobExecuteStatus.NONE);
