	...
	where TContext : IMessageHandlerContext
{
		...
		string? unhandledExceptionDetail)
	{
		...
		try
		{
			...
			if (newResult.HasError)
			{
				...
				catch (Exception onErrorEx)
				{
					try
					{
						handlerContext.LogCritical(traceInfo, null, x => x.ExceptionInfo(onErrorEx), "OnError: Send<Envelope.ServiceBus.Messages.IRequestMessage<TResponse>> error", null);
					}
					catch { }
				}
			}
		...
		catch (Exception exHandler)
		{
			...
			try
			{
				handlerContext.LogError(traceInfo, null, x => x.ExceptionInfo(exHandler), "Send<Envelope.ServiceBus.Messages.IRequestMessage<TResponse>> error", null);
			}
			catch { }
