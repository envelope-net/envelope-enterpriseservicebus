...
internal abstract class AsyncMessageHandlerProcessor<TResponse> : MessageHandlerProcessorBase
{
	public abstract Task<IResult<ISendResponse<TResponse>>> HandleAsync(
		Envelope.ServiceBus.Messages.IRequestMessage<TResponse> message,
...
internal class AsyncMessageHandlerProcessor<TRequestMessage, TResponse, TContext> : AsyncMessageHandlerProcessor<TResponse>
	where TRequestMessage : Envelope.ServiceBus.Messages.IRequestMessage<TResponse>
	where TContext : IMessageHandlerContext
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			if (newResult.HasError)
			{
				...
				catch (Exception onErrorEx)
				{
					try
					{
						await handlerContext.LogCriticalAsync(traceInfo, null, x => x.ExceptionInfo(onErrorEx), "OnErrorAsync: SendAsync<Envelope.ServiceBus.Messages.IRequestMessage<TResponse>> error", null, cancellationToken).ConfigureAwait(false);
					}
					catch { }
				}
			}
		...
		catch (Exception exHandler)
		{
			...
			try
			{
				await handlerContext.LogErrorAsync(traceInfo, null, x => x.ExceptionInfo(exHandler), "SendAsync<Envelope.ServiceBus.Messages.IRequestMessage<TResponse>> error", null, cancellationToken).ConfigureAwait(false);
			}
			catch { }
		...
		catch (Exception onErrorEx)
		{
			try
			{
				if (handlerContext != null)
					await handlerContext.LogCriticalAsync(traceInfo, null, x => x.ExceptionInfo(onErrorEx), "OnErrorAsync: SendAsync<Envelope.ServiceBus.Messages.IRequestMessage> error", null, cancellationToken).ConfigureAwait(false);
			}
			catch { }
		}
	}
