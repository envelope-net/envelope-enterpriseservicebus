	...
	where TContext : IMessageHandlerContext
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		catch (Exception exHandler)
		{
			...
			try
			{
				await handlerContext.LogErrorAsync(traceInfo, null, x => x.ExceptionInfo(exHandler), "SendAsync<Envelope.ServiceBus.Messages.IRequestMessage> error", null, cancellationToken).ConfigureAwait(false);
			}
			catch { }
